...
# ── Validate ──────────────────────────────────────────────────────────────────
for var in IMAGE_NAME KUBERNETES_VERSION REPOSITORY TAG OUTPUT_DIR; do
    if [[ -z "${!var}" ]]; then
...
oras push "$IMAGE_REF" \
    ...
    --annotation "org.opencontainers.image.description=Kubernetes ${KUBERNETES_VERSION} node image (${IMAGE_NAME}) with containerd" \
