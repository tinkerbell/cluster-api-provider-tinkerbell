...
func TestDefaultOption(t *testing.T) {
	...
	t.Run("empty namespace sets no default namespaces", func(t *testing.T) {
		t.Parallel()
		...
		if opts.Cache.DefaultNamespaces != nil {
			t.Fatalf("expected nil DefaultNamespaces, got %v", opts.Cache.DefaultNamespaces)
		}
		if opts.Scheme != s {
			t.Fatal("expected scheme to be set")
		}
		if len(opts.Cache.ByObject) != 2 {
			t.Fatalf("expected 2 ByObject entries, got %d", len(opts.Cache.ByObject))
		}
	})

	t.Run("non-empty namespace sets default namespaces", func(t *testing.T) {
		t.Parallel()

		s := runtime.NewScheme()
		opt := DefaultOption(s, "tinkerbell")

		var opts cluster.Options
		opt(&opts)

		if len(opts.Cache.DefaultNamespaces) != 1 {
			t.Fatalf("expected 1 default namespace, got %d", len(opts.Cache.DefaultNamespaces))
		}
		if _, ok := opts.Cache.DefaultNamespaces["tinkerbell"]; !ok {
			t.Fatal("expected 'tinkerbell' in DefaultNamespaces")
		}
	})
}
